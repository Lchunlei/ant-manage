package com.ant.app.util.sdk;


import com.ant.app.util.sdk.WXPayConstants.SignType;
...
//        }
    ...
    //打钱给指定微信用户
    public Map<String,String> sendMoneyTo(Map<String,String> reqData) throws Exception {
        String msgUUID = reqData.get("nonce_str");
        String nSign = WXPayUtil.generateSignature(reqData,this.config.getKey(),SignType.MD5);
        reqData.put("sign",nSign);
        String reqBody = WXPayUtil.mapToXml(reqData);
        log.info("打钱微信请求--》"+reqBody);
        String resp = this.wxPayRequest.requestWithCert(WXPayConstants.SEND_MONEY_TO_USER, msgUUID, reqBody, config.getHttpConnectTimeoutMs(), config.getHttpReadTimeoutMs(), autoReport);
        log.info("打钱微信响应--》"+resp);
        return WXPayUtil.xmlToMap(resp);
    }
    //查询提现结果
    public Map<String,String> findMoneyTo(Map<String,String> reqData) throws Exception {
        String msgUUID = reqData.get("nonce_str");
        String nSign = WXPayUtil.generateSignature(reqData,this.config.getKey(),SignType.MD5);
        reqData.put("sign",nSign);
        String reqBody = WXPayUtil.mapToXml(reqData);
        log.info("查询提现结果微信请求--》"+reqBody);
        String resp = this.wxPayRequest.requestWithCert(WXPayConstants.SEND_MONEY_TO_USER_RESULT, msgUUID, reqBody, config.getHttpConnectTimeoutMs(), config.getHttpReadTimeoutMs(), autoReport);
        log.info("查询提现结果微信响应--》"+resp);
        return WXPayUtil.xmlToMap(resp);
    }
